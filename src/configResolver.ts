...
const addBuildEntryPoints = (
  ...
  const entryFiles = parseInput(config);
  for (const [entryName, entry] of Object.entries(entryFiles)) {
    ...
    const finalEntryName = isLegacy ? `${entryName}-legacy` : entryName;
    const legacyEntryPoint =
      typeof entryPoints[`${finalEntryName}-legacy`] !== "undefined" ? `${finalEntryName}-legacy` : false;
